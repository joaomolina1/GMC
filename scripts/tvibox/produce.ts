...
const NOT_PROCESSED_RE = /generated by VEO that has been processed/i;
const NOT_PROCESSED_RETRIES = 6;
const NOT_PROCESSED_WAIT_MS = 20_000;
...
async function startOperation(key: string, model: string, body: unknown): Promise<string> {
  for (let attempt = 1; ; attempt++) {
    try {
      ...
      // Logo a seguir a uma geração, o Veo ainda pode estar a pós-processar o vídeo anterior; a extensão
      // é recusada com 400 até ficar pronto. Esperar e voltar a submeter em vez de abortar a cadeia.
      const msg = e instanceof Error ? e.message : String(e);
      if (!NOT_PROCESSED_RE.test(msg) || attempt > NOT_PROCESSED_RETRIES) throw e;
      log(`vídeo anterior ainda em processamento no Veo — nova tentativa ${attempt}/${NOT_PROCESSED_RETRIES} em ${NOT_PROCESSED_WAIT_MS / 1000}s`);
      await new Promise((r) => setTimeout(r, NOT_PROCESSED_WAIT_MS));
    }
  }
}
